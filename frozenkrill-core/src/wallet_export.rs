...
use crate::wallet_description::{
    self, AddressInfo, DerivedAddress, MultiSigWalletDescriptionV0, ScriptType, SigType,
    SingleSigWalletDescriptionV0, WalletVersionType, ZERO_MULTISIG_WALLET_VERSION,
    ZERO_SINGLESIG_WALLET_VERSION, slip132_decode_pub,
};
    ...
    pub fn deserialize(data: BufReader<impl Read>) -> anyhow::Result<Self> {
        ...
        match d.wallet.as_ref() {
            Some(w) if w.as_str() != FROZENKRILL_WALLET => {
                bail!(
                    "Not a json generated by frozenkrill because wallet {w} != {FROZENKRILL_WALLET}"
                )
            }
            Some(_) => {}
            None => bail!("Not a json generated by frozenkrill because there is no wallet field"),
        }
        Ok(d)
    }
...
impl SinglesigJsonWalletPublicExportV0 {
    ...
    pub fn deserialize(reader: BufReader<impl Read>) -> anyhow::Result<Self> {
        ...
        anyhow::ensure!(
            d.version == ZERO_SINGLESIG_WALLET_VERSION,
            "Trying to deserialize singlesig export, got version {} != {ZERO_SINGLESIG_WALLET_VERSION}",
            d.version
        );
...
impl MultisigJsonWalletPublicExportV0 {
    ...
    pub fn deserialize(reader: BufReader<impl Read>) -> anyhow::Result<Self> {
        ...
        anyhow::ensure!(
            d.version == ZERO_MULTISIG_WALLET_VERSION,
            "Trying to deserialize multisig export, got version {} != {ZERO_MULTISIG_WALLET_VERSION}",
            d.version
        );
        let sigtype = SigType::from_str(&d.sigtype)?;
        anyhow::ensure!(
