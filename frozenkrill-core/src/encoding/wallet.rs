...
type OptOrigin = Option<(Fingerprint, DerivationPath)>;
...
mod tests {
    ...
    fn random_sh_sortedmulti_descriptor(
        secp: &Secp256k1<All>,
        rng: &mut impl CryptoRngCore,
    ) -> anyhow::Result<Descriptor<DescriptorPublicKey>> {
    ...
    fn random_pk(secp: &Secp256k1<All>, rng: &mut impl CryptoRngCore) -> secp256k1::PublicKey {
        let kp = bitcoin::key::Keypair::new(secp, rng);
        kp.public_key()
    }

    fn random_xpub(secp: &Secp256k1<All>, rng: &mut impl CryptoRngCore) -> anyhow::Result<Xpub> {
    ...
    fn random_opt_origin() -> anyhow::Result<OptOrigin> {
    ...
    fn random_wildcard() -> anyhow::Result<Wildcard> {
    ...
    fn random_deriv_paths() -> anyhow::Result<DerivPaths> {
    ...
    fn random_derivation_path() -> anyhow::Result<DerivationPath> {
        let n: usize = random::random_u32_in_range(1, 9)?.try_into()?;
        let mut v = Vec::with_capacity(n);
        for _ in 0..n {
    ...
    fn random_child_number() -> anyhow::Result<ChildNumber> {
