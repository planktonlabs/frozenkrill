...
use frozenkrill_core::{
    ...
    wallet_description::{
        ...
        MultisigJsonWalletDescriptionV0, MultisigType, SigType, KEY_SIZE, SALT_SIZE,
    },
    wallet_export::{
...
use frozenkrill_core::wallet_description::{read_decode_wallet, SingleSigWalletDescriptionV0};
...
use crate::{
    ...
    handle_input_path, handle_output_path, ui_derive_key, ui_get_singlesig_wallet_description,
    InternetChecker, InternetCheckerImpl, MultisigOpenArgs, SinglesigOpenArgs,
};

use self::{generate::DuressPublicInfoParams, interactive::ask_for_keyfiles_generate};
...
const CONTEXT_CORRUPTION_WARNING: &str = "failure decrypting wallet, some data has been corrupted on disk or memory, change the destination disk or check the ram memory";
...
pub(super) fn singlesig_core_open(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn multisig_core_open(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
    encrypted_wallet: &EncryptedWalletDescription,
...
fn ask_open_signers(
    ...
    for _ in 0..n {
        ...
        let (wallet, non_duress_password) = crate::commands::singlesig_core_open(
            theme,
            term,
            secp,
...
pub(super) fn open_singlesig_wallet_non_interactive(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
...
pub(super) fn open_multisig_wallet_non_interactive(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
    ic: impl InternetChecker,
...
pub(super) struct AddressGenerationParams {
    pub first_index: u32,
    pub quantity: u32,
}

fn generate_singlesig_public_info(
    secp: &Secp256k1<All>,
...
fn generate_name(prefix: &str, name_salt: &[u8; KEY_SIZE], index: usize, suffix: &str) -> String {
...
fn from_wallet_to_public_info_json_path(wallet_output_path: &Path) -> anyhow::Result<PathBuf> {
    extend_base_name_with_suffix(wallet_output_path, PUB_FILE_SUFFIX)
}

fn from_input_to_signed_psbt(file: &Path) -> anyhow::Result<PathBuf> {
    extend_base_name_with_suffix(file, SIGNED_PSBT_FILE_SUFFIX)
}

fn from_public_info_json_path_to_non_duress(file: &Path) -> anyhow::Result<PathBuf> {
...
pub(super) fn parse_multisig_input(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
    i: &Path,
    ...
    match GenericOutputExportJson::deserialize(utils::buf_open_file(i)?) {
        ...
        Err(_) => {
            ...
            let v = ask_try_decrypt(i, &e, theme, term, secp)?;
            receiving_descriptors.insert(v.receiving_multisig_public_descriptor());
            change_descriptors.insert(v.change_multisig_public_descriptor());
            signers.push(v);
        }
...
pub(super) fn parse_multisig_inputs(
    theme: &dyn Theme,
    term: &Term,
    secp: &Secp256k1<All>,
    ic: impl InternetChecker,
...
fn calculate_non_duress_output(
    enable_duress_wallet: bool,
...
pub(crate) fn non_duress_params_from_password(
    non_duress_password: &Option<Arc<SecretString>>,
    non_duress_output_file_json: &Option<PathBuf>,
    public_json_file_path: &Option<PathBuf>,
) -> anyhow::Result<Option<DuressPublicInfoParams>> {
    Ok(match non_duress_password {
        Some(non_duress_password) => {
            calculate_non_duress_output(true, non_duress_output_file_json, public_json_file_path)?
                .map(
